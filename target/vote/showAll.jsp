...
<script>
    ...
    function to_page(num) {
        $.ajax({
            type: "post",
            url: "/vote_war/vote?method=showAll&time=" + new Date().getTime(),
            dataType: 'json',
            data:{pageNum:num},
    ...
    function build_vote_list(result) {
        ...
        $.each(themes, function (index, item) {
            ...
            var name_td = $("<td></td>").append(item.username);
            var description_td = $("<td></td>").append(item.title);
            var endTime_td = $("<td></td>").append(new Date(item.endTime).format("yyyy-MM-dd hh:mm:ss"));
            ...
            btn_td.append(voteInfo_td).append(delVote_td);
            var tr = $("<tr></tr>").append(id_td).append(name_td).append(description_td).append(endTime_td).append(btn_td);
